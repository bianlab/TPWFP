...
imwrite(uint8(255*(abs(im_r_ini))),[newfolder '/recover_TPWFP_amp_0.jpg'],'jpg');
imwrite(uint8(255*(im_r_ini/max(max(im_r_ini)))),[newfolder '/recover_TPWFP_amp_norm_0.jpg'],'jpg');
...
for t = 1: Params.T,
    ...
    if mod(t,save_sep) == 0
        ...
        imwrite(uint8(255*(abs(im_r))),[newfolder '/recover_TPWFP_amp_' num2str(t) '.jpg'],'jpg');
        imwrite(uint8(255*(abs(im_r)/max(max(abs(im_r))))),[newfolder '/recover_TPWFP_amp_norm_' num2str(t) '.jpg'],'jpg');
        im_r = ifft2(ifftshift(z));
        im_r_ang = angle(im_r)/pi;
        im_r_ang = im_r_ang - min(min(im_r_ang));
        imwrite(uint8(255*(abs(im_r_ang)/max(max(abs(im_r_ang))))),[newfolder '/recover_TPWFP_phase_' num2str(t) '.jpg'],'jpg');
